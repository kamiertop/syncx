...
type poolDequeue[T any] struct {
	...
	// vals is a ring buffer of values stored in this dequeue. The
	// size of this must be a power of 2.
	//
	// vals[i].occupied is 0 if the slot is empty and 1 otherwise.
	// A slot is still in use until *both* the tail index has moved
	// beyond it and occupied has been set to 0. This is set to 0
...
func (d *poolDequeue[T]) popTail() (T, bool) {
	...
	//
	// We write to val first and then publish that we're done with this
	// slot by atomically clearing occupied.
	var zero T
	slot.val = zero
	slot.occupied.Store(0)
